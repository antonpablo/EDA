...
public:
	...
	**/
	...
	La complejidad del algoritmo es de complejidad lineal O(n), en caso de que la posicion sea la ultima del array
	se recorre 1 vez solo el array.

	**/

	void llevarAlPrincipio(unsigned int pos) {
		
		if (pos > _nelems || pos <= 0)
